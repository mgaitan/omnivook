...
def main():
    parser = argparse.ArgumentParser(description="Generate an ebook from Omnivore articles")
    parser.add_argument("--label", type=str, default=LABEL, help="Label to filter articles")
    parser.add_argument("--since", type=lambda s: datetime.strptime(s, '%Y-%m-%d').date(), default=YESTERDAY, help="Start date to filter articles (format YYYY-MM-DD)")
    parser.add_argument("--convert-to-mobi", action="store_true", help="Convert the output to .mobi format")
    parser.add_argument("--archive", action="store_true", help="Archive exported articles")
...
def get_articles(label=LABEL, since=YESTERDAY, archive=False):
    ...
    if archive:
        print("Archiving...")
        for art in articles:
            art.archive_article(art['node']['id'])
    return articles
